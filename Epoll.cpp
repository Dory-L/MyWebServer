...
Epoll::Epoll() : epollFd_(epoll_create1(EPOLL_CLOEXEC)), events_(EVENTSNUM)//epoll_create1(flag),epoll_create(int size)没用的size参数被舍弃
{
    assert(epollFd_ > 0);
}
Epoll::~Epoll() {}
...
void Epoll::epoll_add(SP_Channel request, int timeout)
{
    ...
    fd2chan_[fd] = request;//复制shared_ptr,计数+1
