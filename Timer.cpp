    ...
    : deleted_(false), SPHttpData(requestData)
{
    ...
    // 以毫秒计
    expiredTime_ =
        (((now.tv_sec % 10000) * 1000) + (now.tv_usec / 1000)) + timeout;//换算成微秒
}
...
void TimerNode::update(int timeout)
{
    ...
    gettimeofday(&now, NULL);
    expiredTime_ =
        (((now.tv_sec % 10000) * 1000) + (now.tv_usec / 1000)) + timeout;//换算成微秒（1/1000s）
}
...
bool TimerNode::isValid()
{
    ...
    {
        this->setDeleted();//置位deleted_
        return false;
    }
}

//清零http请求，并将删除节点标志置为1
