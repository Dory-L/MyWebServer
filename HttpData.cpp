...
//获取后缀对应的MIME类型，若无法识别默认为text/html
std::string MimeType::getMime(const std::string &suffix)
{
    pthread_once(&once_control, MimeType::init);//只初始化一次
...
void HttpData::reset()
{
    ...
    if (timer_.lock())
    {
        ...
        my_timer->clearReq();//清楚timer_上的 http请求
        timer_.reset();//重置指针
    }
}

//清除timer_上的http请求，并重置timer_指针
...
void HttpData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<hr><em> LiuLanyi's Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: LiuLanyi's Web Server\r\n";
    ;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
